...
export default function BoardSection() {
  ...
  const filtered =
    activeTab === "all" ? posts : posts.filter((p) => p.카테고리 === activeTab);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {filtered.slice(0, 9).map((post) => (
              ...
              >
                ...
                <div className="p-5">
                  <span
                    className={`inline-block px-3 py-1 rounded-full text-[11px] font-bold text-white mb-3 ${categoryStyle(post.카테고리)}`}
                  >
                    {post.카테고리 || "성공사례"}
                  </span>
