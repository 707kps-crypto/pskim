...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!bucketName || !publicUrl) {
      throw new Error(
        "R2_BUCKET_NAME 또는 R2_PUBLIC_URL이 설정되지 않았습니다.",
      );
    }
    ...
    console.error("[PSKim] Upload thumbnail error:", error);
    return NextResponse.json(
  ...
  try {
    ...
    if (!deleted) {
      return NextResponse.json(
        {
          success: false,
          error: "유효하지 않은 R2 URL이거나 삭제에 실패했습니다.",
        },
        { status: 400 },
      );
    }
    ...
    console.error("[PSKim] Delete thumbnail error:", error);
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 },
    );
