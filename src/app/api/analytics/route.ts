...
export async function GET(request: NextRequest) {
  try {
    ...
    const deviceList = devRows.map((r) => {
      ...
      return {
        type: labelMap[label] || label,
        count,
        percent: calcPercent(count, devTotal),
      };
    });
    ...
    const funnel = [
      ...
      {
        label: "CTA 클릭",
        value: conversions.cta_click,
        eventName: "cta_click",
      },
      {
        label: "폼 노출",
        value: conversions.form_visible,
        eventName: "form_visible",
      },
      {
        label: "입력 시작",
        value: conversions.form_start,
        eventName: "form_start",
      },
      {
        label: "폼 제출",
        value: conversions.form_submit,
        eventName: "form_submit",
      },
    ];

    // Page flow
    const flowNodes: { page: string; views: number }[] = topPages
      .slice(0, 8)
      .map((p) => ({
        page: p.path,
        views: p.views,
