    ...
    if (썸네일 !== undefined) {
      try {
        ...
        if (oldRes.ok) {
          ...
          const oldThumbnail = oldRecord.fields?.[FIELD_IDS.썸네일] as
            | string
            | undefined;
        ...
        console.error("[PSKim] Failed to cleanup old thumbnail:", e);
      }
    }
  ...
  try {
    ...
    try {
      ...
      if (getRes.ok) {
        ...
        const thumbnail = record.fields?.[FIELD_IDS.썸네일] as
          | string
          | undefined;
        if (thumbnail) {
      ...
      console.error("[PSKim] Failed to cleanup thumbnail on delete:", e);
    }
