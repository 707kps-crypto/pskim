...
const POPUPS_TABLE_ID = process.env.AIRTABLE_POPUPS_TABLE_ID || "";
...
export async function GET(request: NextRequest) {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      if (!all) {
        queryOptions.filterByFormula = `AND({활성여부} = TRUE(), OR({시작일} = '', {시작일} <= '${today}'), OR({종료일} = '', {종료일} >= '${today}'))`;
      }
    ...
    console.error("[PSKim] Popups GET error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const record = await base(POPUPS_TABLE_ID).create(
      fields as Partial<FieldSet>,
    );

    return NextResponse.json({ success: true, id: record.id });
  } catch (error) {
    console.error("[PSKim] Popups POST error:", error);
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 },
    );
  ...
  try {
    ...
    if (updateData.이미지URL !== undefined) {
      try {
        ...
        console.error("[PSKim] Failed to cleanup old popup image:", e);
      }
    }
    ...
    if (updateData.ALT텍스트 !== undefined)
      fields["ALT텍스트"] = updateData.ALT텍스트;
    if (updateData.이미지URL !== undefined)
      fields["이미지URL"] = updateData.이미지URL;
    if (updateData.링크URL !== undefined)
      fields["링크URL"] = updateData.링크URL;
    if (updateData.링크타겟 !== undefined)
      fields["링크타겟"] = updateData.링크타겟;
    if (updateData.순서 !== undefined) fields["순서"] = Number(updateData.순서);
    if (updateData.활성여부 !== undefined)
      fields["활성여부"] = updateData.활성여부;
    ...
    console.error("[PSKim] Popups PUT error:", error);
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 },
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("[PSKim] Failed to cleanup popup image on delete:", e);
    }
    ...
    console.error("[PSKim] Popups DELETE error:", error);
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 },
    );
