...
async function deleteR2ImageClient(url: string): Promise<void> {
  ...
  try {
    ...
    console.error("[PSKim] Failed to delete R2 image:", e);
  }
}
...
function getPopupStatus(popup: Popup): { label: string; color: string } {
  ...
  if (!popup.활성여부)
    return { label: "비활성", color: "bg-white/[0.06] text-gray-500" };

  if (popup.시작일 && popup.시작일 > today)
    return { label: "예약", color: "bg-yellow-100 text-yellow-700" };

  if (popup.종료일 && popup.종료일 < today)
...
export default function PopupsPage() {
  ...
  const setField = useCallback(
    <K extends keyof PopupForm>(key: K, value: PopupForm[K]) => {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/popups?id=${deleteTarget.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          PC에서는 최대 4개까지 노출되며, 모바일에서는 스와이프로 확인
          가능합니다.
        </p>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-64 rounded-2xl bg-white/[0.06] animate-pulse"
            />
          ))}
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="relative aspect-[4/3] bg-white/[0.06]">
                  ...
                  {/* 상태 배지 */}
                  <span
                    className={`absolute top-2 right-2 text-[11px] px-2 py-0.5 rounded-full font-medium ${status.color}`}
                  >
                    {status.label}
                  </span>
                </div>
